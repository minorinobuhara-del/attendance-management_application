@extends('layouts.admin') {{-- 管理者ヘッダー用 --}}
...
<div class="detail-card">
    ...
    <tr>
        ...
        <td>
        @if($pendingRequest)
            <span class="detail-strong">{{ $attendance->clock_in ? \Carbon\Carbon::parse($attendance->clock_in)->format('H:i') : '' }}</span>
            <span class="time-tilde">～</span>
            <span class="detail-strong">{{ $attendance->clock_out ? \Carbon\Carbon::parse($attendance->clock_out)->format('H:i') : '' }}</span>
        @else
            <div class="time-row">
            <input class="time-input" name="clock_in" value="{{ old('clock_in', $attendance->clock_in ? \Carbon\Carbon::parse($attendance->clock_in)->format('H:i') : '') }}">
            <span class="time-tilde">～</span>
            <input class="time-input" name="clock_out" value="{{ old('clock_out', $attendance->clock_out ? \Carbon\Carbon::parse($attendance->clock_out)->format('H:i') : '') }}">
            </div>
        @endif
        </td>
    </tr>
